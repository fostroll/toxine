...
def add_brat_text_bound_annotations(txt_fn, ann_fn, save_to=None):
    """Converts txt and ann files generated by brat to text file with embedded
    annotations.
    ...
    param save_to: a path where the result will be stored. If ``None``
                   (default), the funtion return the result as a generator of
                   text data
    """
    def process():
        anns = []
...
def postprocess_brat_conllu(corpus, save_to=None):
    """Converts corpus with in txt format into CoNLL-U format. Embedded brat
...
def make_ne_tags(corpus, save_to=None):
    """Replace brat entities in the corpus in CoNLL-U or Parsed CoNLL-U format
    to MISC:NE entities supported by mordl. Note, that if several brat
...
def brat_to_ne(txt_fn, ann_fn, save_to=None):
    """Converts txt and ann files generated by brat to CoNLL-U file with named
    entities placed to the NE feat of the MISC field. Note, that if several
    brat entities are linked to the one token, only first one will be used.
